...
动态工作流是一个 JavaScript 脚本，可大规模编排[子代理](002-sub-agents.md)。Claude 为您描述的任务编写脚本，运行时在后台执行它，同时您的会话保持响应。
...
[子代理](002-sub-agents.md)、[skills](https://code.claude.com/docs/zh-CN/skills)、[agent teams](004-agent-teams.md) 和工作流都可以运行多步骤任务。区别在于谁掌握计划：
...
查看工作流运行的最快方式是运行 `/deep-research`，这是 Claude Code 包含的[内置工作流](#%E6%8D%86%E7%BB%91%E5%B7%A5%E4%BD%9C%E6%B5%81)，用于跨许多来源调查问题。您将看到代理在后台通过一组阶段工作，同时您的会话保持空闲，最后获得一份报告而不是逐轮记录。
...
<Steps>
  ...
  <Step title="允许工作流">
    Claude Code 询问是否允许工作流。选择**是**继续。确切的提示取决于您的权限模式。有关每种模式的选项，请参阅[在运行前批准计划](#%E5%9C%A8%E8%BF%90%E8%A1%8C%E5%89%8D%E6%89%B9%E5%87%86%E8%AE%A1%E5%88%92)。
  </Step>
  ...
  <Step title="观看进度">
    ...
    该视图显示每个阶段及其代理计数、令牌总数和经过的时间。深入任何阶段以查看其代理及每个代理发现的内容。有关完整的控制集，请参阅[观看运行](#%E8%A7%82%E7%9C%8B%E8%BF%90%E8%A1%8C)。
...
要为您自己的任务运行工作流，[让 Claude 编写一个](#%E8%AE%A9%20Claude%20%E7%BC%96%E5%86%99%E5%B7%A5%E4%BD%9C%E6%B5%81)，一旦运行完成您想要的操作，您可以[保存它](#%E4%BF%9D%E5%AD%98%E5%B7%A5%E4%BD%9C%E6%B5%81%E4%BB%A5%E4%BE%9B%E9%87%8D%E7%94%A8)作为您自己的命令。
...
| 命令                          | 它做什么                                                                                                                                 |
| :-------------------------- | :----------------------------------------------------------------------------------------------------------------------------------- |
| `/deep-research <question>` | 在多个角度上扇出网络搜索问题，获取并交叉检查它找到的来源，对每个声明投票，并返回一份引用的报告，其中未通过交叉检查的声明已被过滤掉。需要[WebSearch 工具](https://code.claude.com/docs/zh-CN/tools-reference#websearch-tool-behavior)可用 |

[您自己保存的工作流](#%E4%BF%9D%E5%AD%98%E5%B7%A5%E4%BD%9C%E6%B5%81%E4%BB%A5%E4%BE%9B%E9%87%8D%E7%94%A8)以相同方式成为命令，并在 `/` 自动完成中与捆绑的工作流一起出现。
...
| `r`           | 重启选定的运行中代理                                                  |
| `s`           | [保存](#%E4%BF%9D%E5%AD%98%E5%B7%A5%E4%BD%9C%E6%B5%81%E4%BB%A5%E4%BE%9B%E9%87%8D%E7%94%A8)运行的脚本作为命令                 |
...
* [在您的提示中请求工作流](#%E5%9C%A8%E6%82%A8%E7%9A%84%E6%8F%90%E7%A4%BA%E4%B8%AD%E8%AF%B7%E6%B1%82%E5%B7%A5%E4%BD%9C%E6%B5%81)，使用关键字 `ultracode`，Claude 为任务编写一个。
* [让 Claude 使用 ultracode 决定](#%E8%AE%A9%20Claude%20%E4%BD%BF%E7%94%A8%20ultracode%20%E5%86%B3%E5%AE%9A)：设置 `/effort ultracode`，Claude 为会话中的每个实质性任务规划工作流。

您也可以运行已存在的工作流命令：一个[捆绑工作流](#%E6%8D%86%E7%BB%91%E5%B7%A5%E4%BD%9C%E6%B5%81)如 `/deep-research`，或一个您已[保存](#%E4%BF%9D%E5%AD%98%E5%B7%A5%E4%BD%9C%E6%B5%81%E4%BB%A5%E4%BE%9B%E9%87%8D%E7%94%A8)的。
...
如果运行完成了您想要的操作，您可以之后[将其保存为命令](#%E4%BF%9D%E5%AD%98%E5%B7%A5%E4%BD%9C%E6%B5%81%E4%BB%A5%E4%BE%9B%E9%87%8D%E7%94%A8)。
...
Ultracode 持续当前会话，当您启动新会话时重置。当您返回日常工作时，使用 `/effort high` 下降。它在支持 `xhigh` [努力](https://code.claude.com/docs/zh-CN/model-config#adjust-effort-level)的模型上可用；在其他模型上，`/effort` 菜单不提供它。
...
您是否看到此提示取决于您的[权限模式](https://code.claude.com/docs/zh-CN/permission-modes)：
...
您的权限模式仅控制上面的启动提示。工作流生成的子代理始终在 `acceptEdits` 模式下运行，并继承您的[工具允许列表](https://code.claude.com/docs/zh-CN/settings#permission-settings)，无论您的会话模式如何。文件编辑自动批准。
...
* `~/.claude/workflows/` 在您的主目录中：在每个项目中可用，仅对您可见。如果您设置了 [`CLAUDE_CONFIG_DIR`](https://code.claude.com/docs/zh-CN/env-vars)，此位置是该路径下的 `workflows/` 目录。
...
当您[保存工作流](#%E4%BF%9D%E5%AD%98%E5%B7%A5%E4%BD%9C%E6%B5%81%E4%BB%A5%E4%BE%9B%E9%87%8D%E7%94%A8)时，`.claude/workflows/` 中的文件包含一个 `meta` 块，后跟编排子代理的脚本体。您通常不需要编辑它，但这是一个小的形状，所以您可以识别 Claude 生成的内容：
...
主体是带有顶级 `await` 的纯 JavaScript。`agent()` 生成一个子代理，`pipeline()` 为列表中的每个项目运行一个。如果您想手动编辑脚本，要求 Claude 引导您完成更改，或查看 [Agent SDK 参考](https://code.claude.com/docs/zh-CN/agent-sdk/typescript)中的 Workflow 工具条目以获取完整的选项集。
...
运行时在运行进行时跟踪每个代理的结果，这是使运行在同一会话中[可恢复](#%E6%9A%82%E5%81%9C%E5%90%8E%E6%81%A2%E5%A4%8D)的原因。
...
要在提交大型任务前评估支出，请先在小范围上运行工作流：一个目录而不是整个仓库，或一个狭窄的问题而不是一个宽泛的问题。`/workflows` 视图显示每个代理的令牌使用情况，随着运行进行，您可以随时在那里停止运行而不会丢失已完成的工作。运行时的[代理上限](#%E8%A1%8C%E4%B8%BA%E5%92%8C%E9%99%90%E5%88%B6)限制单次运行可以生成多少个代理，这限制了失控脚本的成本。要默认保持每次运行更小，在 `/config` 中[设置大小指南](#%E8%AE%BE%E7%BD%AE%E5%A4%A7%E5%B0%8F%E6%8C%87%E5%8D%97)。

Claude Code 还会标记增长异常大的运行。当工作流调度超过 25 个代理，或其预计令牌总数超过 150 万时，输入框下方任务面板中的其进度行显示 `Large workflow` 警告。警告指向您可以停止运行的 [`/workflows`](#%E8%A7%82%E7%9C%8B%E8%BF%90%E8%A1%8C)。需要 Claude Code v2.1.203 或更高版本。

警告是建议性的：它不会暂停或限制运行。当您看到它时，两个设置会改变：

* 如果您[设置大小指南](#%E8%AE%BE%E7%BD%AE%E5%A4%A7%E5%B0%8F%E6%8C%87%E5%8D%97)，指南的代理计数替换 25 个代理的阈值。
* 启用[ultracode](#%E8%AE%A9%20Claude%20%E4%BD%BF%E7%94%A8%20ultracode%20%E5%86%B3%E5%AE%9A)的会话不显示警告，因为打开 ultracode 已经让您选择加入大型运行。

工作流中的每个代理使用您的会话模型，除非脚本将阶段路由到不同的模型，或设置了 [`CLAUDE_CODE_SUBAGENT_MODEL`](https://code.claude.com/docs/zh-CN/model-config#environment-variables) 环境变量，该变量会覆盖两者。要控制模型成本：
...
更改在下一个提示时生效。[运行时代理上限](#%E8%A1%8C%E4%B8%BA%E5%92%8C%E9%99%90%E5%88%B6)仍然适用，无论设置如何。

<h3 id="turn-workflows-off">
  关闭工作流
</h3>

工作流在 CLI、桌面应用、IDE 扩展、[非交互模式](https://code.claude.com/docs/zh-CN/headless)与 `claude -p` 和 [Agent SDK](https://code.claude.com/docs/zh-CN/agent-sdk/overview) 中可用。相同的禁用设置在每个表面上应用。
...
要为整个组织关闭工作流，在[托管设置](https://code.claude.com/docs/zh-CN/server-managed-settings)中设置 `"disableWorkflows": true`，或使用[Claude Code 管理员设置](https://claude.ai/admin-settings/claude-code)页面上的切换。
...
* [并行运行代理](001-agents.md)：比较子代理、代理视图、代理团队和工作流
* [创建自定义子代理](002-sub-agents.md)：工作流编排的工作者原语
* [管理成本](https://code.claude.com/docs/zh-CN/costs)：多代理运行如何计入使用限制

<!-- created: 2026-08-19 12:59:32 -->
<!-- updated: 2026-08-19 13:18:52 -->