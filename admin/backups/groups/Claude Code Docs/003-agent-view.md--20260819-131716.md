...
名称用该会话中由 [`/color`](https://code.claude.com/docs/zh-CN/commands) 设置的颜色着色。从 v2.1.199 开始，当你用 `←` 或 `/background` [后台会话](#%E4%BB%8E%E4%BC%9A%E8%AF%9D%E5%86%85%E9%83%A8)时，颜色会保留。
...
整个输出不包含字母或数字的回合，例如打印单个符号的安静迭代的 [`/loop`](https://code.claude.com/docs/zh-CN/scheduled-tasks) 会话，保持行的前一个摘要和状态。在 v2.1.205 之前，该回合被重新分类，可能将等待你输入的会话翻转回 `Working`。

结束回合摘要和每次中途重写是通过你的正常提供商的一个短 Haiku-class 请求，按与会话本身相同的[数据使用条款](https://code.claude.com/docs/zh-CN/data-usage)计费和处理。15 秒的模型重写之间的更新重用会话自己的输出，不发送请求。在第三方提供商（如 Amazon Bedrock、Google Cloud 的 Agent Platform、Microsoft Foundry 和自定义网关）上，当没有配置 Haiku 模型时，请求会回退到会话的主模型。设置 [`ANTHROPIC_DEFAULT_HAIKU_MODEL`](https://code.claude.com/docs/zh-CN/model-config#environment-variables) 以在这些提供商上为这些摘要选择模型。
...
后台会话拒绝 `/install-github-app` 和 [`/mcp`](https://code.claude.com/docs/zh-CN/mcp) 设置列表，包括其身份验证操作，无论你是附加还是从窥视面板回复。消息指导你到常规 `claude` 会话，`/mcp reconnect <server>`、`/mcp enable` 和 `/mcp disable` 仍然有效。
...
如果在你按 `←` 时工具正在运行，Claude Code 会等待大约十秒钟让它完成，然后后台，响应在后台会话中继续。再按一次 `←` 以立即后台而不是等待。当进行中的工作无法转移到后台会话时，`Background this session?` 对话首先出现，与 [`/background`](#%E4%BB%8E%E4%BC%9A%E8%AF%9D%E5%86%85%E9%83%A8) 相同。
...
删除也会从[监督进程](#%E7%9B%91%E7%9D%A3%E8%BF%9B%E7%A8%8B)的会话列表中清除会话，无论你用 `Ctrl+X` 删除还是从 shell 用 [`claude rm`](#%E4%BB%8E%20shell%20%E7%AE%A1%E7%90%86%E4%BC%9A%E8%AF%9D) 删除，所以删除在监督进程重启中保持。在 v2.1.206 之前，在监督进程重启或无法访问时删除会话会将其留在该列表中，下一个监督进程重启其进程并再次显示该行。
...
* 从 shell 用 [`claude rm`](#%E4%BB%8E%20shell%20%E7%AE%A1%E7%90%86%E4%BC%9A%E8%AF%9D) 删除会保留有未提交更改的 worktree，以及其会话行。
...
[subagent](002-sub-agents.md) 后台会话生成的继承会话的工作目录，所以其文件编辑落在会话的 worktree 中而不是你的工作副本。要给 subagent 其自己的单独 worktree，在其 frontmatter 中设置 [`isolation: worktree`](002-sub-agents.md#%E6%94%AF%E6%8C%81%E7%9A%84%20frontmatter%20%E5%AD%97%E6%AE%B5) 或在生成它时传递 `isolation: "worktree"`。
...
云提供商选择，如 `CLAUDE_CODE_USE_BEDROCK` 或 `CLAUDE_CODE_USE_VERTEX`，以及 `ANTHROPIC_DEFAULT_*_MODEL` 别名遵循调度会话的 shell。如果你在该 shell 中导出 [`CLAUDE_CODE_EXTRA_BODY`](https://code.claude.com/docs/zh-CN/env-vars) 请求体覆盖，它会以相同的方式到达会话。在 v2.1.206 之前，后台工作进程忽略了 shell 导出的 `CLAUDE_CODE_EXTRA_BODY`。
...
你用 [`/rename`](https://code.claude.com/docs/zh-CN/commands) 或 `Ctrl+R` 设置的名称也会在该重新启动中持续，所以 [`claude --resume <name>`](https://code.claude.com/docs/zh-CN/sessions#name-your-sessions) 仍然解析会话。在 v2.1.202 之前，重新启动会将会话恢复为调度时的名称，新名称停止解析。
...
删除会话会停止它交付的所有内容。要让会话的所有后台工作随进程停止而不是被交付，将 [`CLAUDE_CODE_DISABLE_BG_EXIT_HANDOFF`](https://code.claude.com/docs/zh-CN/env-vars#variables) 环境变量设置为 `1`。
