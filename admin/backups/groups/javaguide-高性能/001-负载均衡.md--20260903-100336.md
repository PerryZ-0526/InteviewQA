...
> ### **它为什么这么快？**
>
> Nginx的高性能主要得益于其`事件驱动`和`异步非阻塞`的架构。它不像传统服务器（如Apache）为每个请求创建一个线程或进程，而是用<u style="text-decoration-color: #e63946">少量**工作进程（Worker）** 处理大量并发连接</u>，资源消耗极低却能支持高达**50,000**个并发连接。
>
> ### **常见使用场景**
>
