...
区别主要在工程抽象上，Hermes 是在原有 Tool 系统里增加一个特殊的 `execute_code` 工具，而 dsh 更进一步把 Code Mode 做成 ToolRuntime 的一种一等呈现模式。对我来说，Hermes 这部分最核心的一句话就是：**不要让 LLM 充当低效的流程解释器，而是让 LLM 写流程，让程序跑流程。**
