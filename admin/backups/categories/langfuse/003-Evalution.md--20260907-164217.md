...
> 在现在的 Langfuse v4 里，生产环境最推荐的是 Observation-level evaluator，而不是旧的 Trace-level evaluator。原因是 Agent 本身就是由多个可观测步骤组成的，不同节点需要不同的评测标准，Observation 级既能精确定位问题，也能只针对关键节点采样评测，降低 LLM Judge 的成本，同时更适合实时高吞吐的线上评测。如果要评价整个 Agent 请求，也不是退回 Trace evaluator，而是给整个 Agent invocation 建一个 root Observation，在这个根节点上评 task success、overall correctness；内部 Retriever、Planner、Tool、Answer Generator 再分别做细粒度评测。这样既保留端到端指标，又能向下定位具体失败环节。
>
> ```
