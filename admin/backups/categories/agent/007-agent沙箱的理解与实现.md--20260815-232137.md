...
<span style="background-color: #fff3cd">攻击者可以把注入指令藏在这些内容里（**prompt injection**），诱导模型生成恶意代码</span>。E2B 等沙箱厂商的威胁模型直接假设「沙箱内的代码具有敌意」，Manus、Perplexity 等产品选择微虚拟机而非容器，正是因为其威胁模型（注入逃逸、资源滥用、横向移动）要求强隔离。
