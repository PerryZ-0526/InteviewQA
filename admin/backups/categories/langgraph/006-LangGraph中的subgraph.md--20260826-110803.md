...
- 第二，两者都可以形成层级结构。LangGraph 可以出现 Parent Graph → Child Graph → Grandchild Graph 的多层嵌套；Claude Code 的 Subagent 现在也可以继续派生 Subagent，因此都能够构建树状或者层级式任务分解结构。([Docs by LangChain](https://docs.langchain.com/oss/python/langgraph/use-subgraphs "Subgraphs - Docs by LangChain")) 
