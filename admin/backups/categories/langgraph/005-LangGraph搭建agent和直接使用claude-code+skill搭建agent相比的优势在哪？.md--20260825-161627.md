...
Claude Code 的 Skill、MCP、Subagent 实际上都是在增强这个循环。Skill 给模型“怎么做”的知识；MCP 给模型“能做什么”的工具；Subagent 让模型把某些任务委派给独立上下文；Hook 则在生命周期中的确定位置加入程序逻辑。Claude Code 官方现在甚至允许 Hook 执行命令、HTTP 请求、MCP 工具、单轮模型判断，以及实验性的 Agent Hook，所以它的确定性控制能力已经比早期强很多。([Claude](https://code.claude.com/docs/en/hooks?utm_source=chatgpt.com "Hooks reference - Claude Code Docs"))
