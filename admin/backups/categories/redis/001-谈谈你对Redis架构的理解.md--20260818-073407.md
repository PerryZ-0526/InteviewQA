...
> Redis 的事件循环主要处理文件事件和时间事件，其中客户端连接的读写属于文件事件。为了让单线程能够同时管理大量 Socket，Redis 会根据操作系统选择 I/O 多路复用接口，例如 Linux 使用 `epoll`、BSD/macOS 使用 `kqueue`，也可以退化使用 `select`。这些机制的核心都是让一个线程同时监听大量文件描述符，只在连接真正就绪时处理它们。
...
**文件事件**本质上是和文件描述符 `fd` 相关的 I/O 事件，在 Redis 里最主要就是网络连接事件。典型包括：监听 Socket 上出现新连接、客户端 Socket 变得可读、客户端 Socket 变得可写等。比如客户端发送 `GET key` 后，对应连接变成“可读”，事件循环发现这个事件，就读取请求、解析并执行命令；执行完成后如果有响应需要返回，则关注“可写”事件，把结果写回客户端。Redis 会通过 `epoll`、`kqueue`、`select` 等 I/O 多路复用机制，同时监听大量连接，而不是每个连接单独分配一个线程。

**时间事件**是到达某个时间点或者周期性触发的内部任务，不依赖客户端 Socket 是否可读写。比如 Redis 服务器内部需要周期性执行的一些维护工作，都可以通过时间事件驱动。Redis 的事件循环会在处理文件事件之外，同时检查是否有时间事件到期并执行对应回调。所以可以把整个模型简单理解成：
```
Redis Event Loop
      │
      ├── 文件事件
