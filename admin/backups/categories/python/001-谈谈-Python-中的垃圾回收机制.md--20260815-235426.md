...
每个 CPython 对象头都带有 ob\_refcnt 引用计数字段，赋值、传参、容器插入等引用变更都经过 Py\_INCREF/Py\_DECREF 增减计数，归零时立即进入 tp\_dealloc 析构路径。优点是回收及时、行为确定，文件句柄等资源在多数场景下随引用归零立刻释放；代价是每次引用变更都要付出计数操作的开销，且无法处理自引用或互引的容器环——环内对象互相持有引用，计数永不归零，这部分交给循环 GC。另外从 3.12 起（PEP 683），None、True、False、Ellipsis、NotImplemented 以及静态类型、小整数等运行时全局对象被标记为不朽对象，INCREF/DECREF 对它们退化为空操作，sys.getrefcount 会显示天文数字，这是 3.12 以后写引用计数相关测试和 C 扩展时的常见坑。
