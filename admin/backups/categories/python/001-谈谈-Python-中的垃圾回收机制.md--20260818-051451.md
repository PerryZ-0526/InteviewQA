...
> - 分代垃圾回收是 CPython 为处理循环引用设计的机制，它会根据`对象存活时间`把对象划分为不同代，新创建的对象先进入`年轻代`，年轻代检查频率高；`多次回收后仍存活的对象`会晋升到`老年代`，检查频率降低。其依据是“大多数对象生命周期很短”，这样可以减少每次都扫描全部对象的开销；
> - 引用计数负责及时回收普通对象，分代回收主要补充清理循环引用。

每个 CPython 对象头都带有 `ob_refcnt 引用计数字段`，赋值、传参、容器插入等引用变更都经过 Py\_INCREF/Py\_DECREF 增减计数，归零时立即进入 tp\_dealloc 析构路径。

- 优点是回收及时、行为确定，文件句柄等资源在多数场景下随引用归零立刻释放；
- 代价是每次引用变更都要付出计数操作的开销，且无法处理自引用或互引的容器环——环内对象互相持有引用，计数永不归零，这部分交给循环 GC。

另外从 3.12 起（PEP 683），None、True、False、Ellipsis、NotImplemented 以及静态类型、小整数等`运行时全局对象`被标记为`不朽对`象，INCREF/DECREF 对它们退化为空操作，sys.getrefcount 会显示天文数字，这是 3.12 以后写引用计数相关测试和 C 扩展时的常见坑。

循环 GC 并不会扫描所有对象，<span style="background-color: #fff3cd">只跟踪可能形成环的容器对象</span>——即带 Py\_TPFLAGS\_HAVE\_GC 标志、实现 tp\_traverse 的类型。原子类型的实例不被跟踪，只含原子元素的 tuple、只含原子键值的 dict 也被优化为不跟踪，因为它们不可能参与环，可以用 gc.is\_tracked 验证。算法上，收集器通过 update\_refs 与 subtract\_refs 两遍模拟减去环的内部引用，找出外部不可达的环集合，而不是依赖真实引用计数。

代际模型方面，当前 CPython（3.14.5 及以后）恢复`三代分代收集`：新对象进第 0 代，<span style="background-color: #fff3cd">每次存活晋升一代</span>，第 0 代收集最频繁。默认阈值为 (700, 10, 10)，即分配减释放的差值超过 700 触发第 0 代收集，第 0 代收集超过 10 次才检查第 1 代，以此类推；gc.collect() 手动触发全量收集并清空部分类型的 free list。
...
此外，3.13 起提供的`自由线程构建`（PEP 703，3.14 起正式支持）有独立设计：引用计数改为偏向引用计数加延迟引用计数，帧栈上的引用延迟到 GC 阶段处理，堆类型使用每线程引用计数；循环 GC 退化为单代收集器，且每轮需要两次 stop-the-world 暂停。

### 工程上最常见的误区是把 RSS 高当作泄漏

CPython 中小对象（不超过 512 字节）走 pymalloc：64 位平台以 1MiB arena（32 位 256KiB）为单位向系统申请内存，内部切分为 4KiB pool 和按 8 字节对齐的 size class；对象释放后内存留在 pool 和 arena 里复用，只有整个 arena 完全清空才可能归还系统，加上 float 等类型的 free list 与系统分配器自身的保留策略，RSS 不立即下降是正常现象。怀疑泄漏时应先用 gc.get\_count、gc.get\_stats 观察各代收集情况，用 gc.is\_tracked 确认对象是否被跟踪，用 tracemalloc 快照对比定位分配点，必要时用 gc.DEBUG\_SAVEALL 抓取不可回收对象分析引用链。gc.disable 只应在受控场景使用——要么确认代码不产生引用环，要么配合 fork 做写时复制优化：父进程早期 gc.disable、fork 前 gc.freeze 冻结已跟踪对象、子进程再 gc.enable，Instagram 曾以此降低预 fork 架构的内存开销；随意关闭 GC 只会让环持续堆积。
...
对象释放后 block 回到 pool 的空闲链表，pool 回到 arena；只有整个 arena 完全清空才可能归还系统。部分类型还有 **free list**（如 float），小对象释放后直接挂回类型私有链表。超过 512 字节的请求交给系统分配器，而 glibc 等自身也会保留空闲区。三层叠加，RSS 呈「高水位」形态是正常现象，详见 [001-python与C++内存管理](../os/001-python%E4%B8%8EC++%E5%86%85%E5%AD%98%E7%AE%A1%E7%90%86.md) 与 [002-从OS底层到语言层的内存管理全景](../os/002-%E4%BB%8EOS%E5%BA%95%E5%B1%82%E5%88%B0%E8%AF%AD%E8%A8%80%E5%B1%82%E7%9A%84%E5%86%85%E5%AD%98%E7%AE%A1%E7%90%86%E5%85%A8%E6%99%AF.md)。
