...
每个进程有相对独立的地址空间，可以++真正利用多个 CPU 核++，因此适合纯 Python 的计算密集型任务。([Python documentation](https://docs.python.org/3/library/multiprocessing.html?utm_source=chatgpt.com "multiprocessing — Process-based parallelism"))

### 2. Python 多线程：适合`同步阻塞 I/O`
...
def call_sdk(task):
    return legacy_sdk.execute(task)

with ThreadPoolExecutor(10) as pool:
    results = pool.map(call_sdk, tasks)
```

尤其适合：
```
第三方 SDK 只有同步接口（有异步接口的话，建议用协程）
同步数据库驱动
阻塞式文件/网络接口
历史代码不方便改造成 async
```

### 3. Python 多协程：适合`高并发的异步 I/O`
...
现代 Python 后端在“高并发 I/O”场景下越来越偏向协程，但进程、线程、协程各有自己的优势场景。

尤其是 FastAPI、爬虫、网关、Agent 这类大量调用外部 API 的系统，协程确实很常见，因为这些任务大部分时间都在等待网络。<span style="background-color: #fff3cd">但如果整个项目大量依赖同步 SDK、阻塞式数据库驱动、文件接口等，线程依然很实用</span>。
