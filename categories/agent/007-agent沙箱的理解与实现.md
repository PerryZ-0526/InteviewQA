...
← [006-agent长时间循环的注意事项](006-agent长时间循环的注意事项) | [如何防止prompt注入](如何防止prompt注入) →
...
OS 原生沙箱无守护进程、毫秒级生效、可逐路径逐 syscall 精确控制，<span style="background-color: rgb(255, 243, 205)">代价是实现与平台强绑定（三平台三套代码）且部分内核特性受限</span>（如 WSL1 不支持、Ubuntu 24.04 需调 sysctl）。
