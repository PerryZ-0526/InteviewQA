...
> ```
>
> 你可能发现每一轮单独看都不错，但整场对话存在“反复问已经知道的信息”、“前后建议矛盾”、“忘记用户之前提供的信息”等问题。此时就可以给整个 Session 挂：conversation\_quality = 0.82 或者 goal\_completion = true；
>
...
> ---
>
> 最后是 `DatasetRun`。它<u style="text-decoration-color: #e63946">主要用于离线 Experiment</u>。比如你的回归 Dataset 有 200 个测试 case：
>
> ```text
...
> ```
>
> 对于多 Agent 系统尤其重要，因为同一个质量问题可以在不同层级观察。比如最终 `Trace task_success=0`，你继续往下看可能发现是 `RAG Observation relevance=0.2`，而不是 Orchestrator 出错；或者单轮 Trace 都成功，但整个 Session 的 `consistency=0.5`，说明问题出在跨轮记忆。
>
> 也就是说，Score 挂在哪一层，本质上是在回答一个问题：**“你现在到底是在评价一个执行步骤、一次完整任务、一整段对话，还是一整轮实验？”**
...
> 在现在的 Langfuse v4 里，生产环境最推荐的是 Observation-level evaluator，而不是旧的 Trace-level evaluator。原因是 Agent 本身就是由多个可观测步骤组成的，<u style="text-decoration-color: #e63946">不同节点需要不同的评测标准，Observation 级既能精确定位问题，也能只针对关键节点采样评测，降低 LLM Judge 的成本，同时更适合实时高吞吐的线上评测</u>。
>
> 如果要评价整个 Agent 请求，也不是退回 Trace evaluator，而是给整个 Agent invocation 建一个 `root Observation`，在这个根节点上评 task success、overall correctness；内部 Retriever、Planner、Tool、Answer Generator 再分别做细粒度评测。这样既保留端到端指标，又能向下定位具体失败环节。
>
> Root Observation 是 Trace 中的一个特殊 Observation，它<u style="text-decoration-color: #e63946">代表一次应用或 Agent 调用的逻辑入口</u>；Trace 则是所有共享同一 trace\_id 的 Observation 组成的完整调用树。Root Observation 可以承载整个 Agent 的 input/output，所以在评测语义上能够替代很多过去的 Trace-level evaluator 场景，但它本身并不自动包含子 Observation 的详细执行数据。
>
> ```
> 线上默认：Observation-level
> 整体任务：Root Observation
> 内部节点：具体 Observation
> 离线版本比较：Experiment / DatasetRun
> ```
>
> 这套粒度划分现在最符合 Langfuse v4 的设计方向。

附：为什么trace observation被弃用了？

Trace-level evaluator 被弃用，核心原因不是“Trace 级评测没价值了”，而是 Langfuse v4 整个底层数据模型从过去的 **trace-centric** 转成了 **observations-first**。在新模型里，Trace 不再是一个拥有自己 input/output 的独立实体，而更像一个 `trace_id`，负责把一组 Observation 组织成一条调用链；真正的输入、输出、元数据都落在 Observation 上，因此继续维护一套独立的 Trace evaluator 会和新的数据模型重复甚至冲突。相比之下，Observation-level evaluator 可以直接对真实执行节点做实时评测，既能评整个 Agent 的 root observation，也能评内部 Retriever、Planner、Tool Call、Generation 等具体步骤，而且在大 Trace、高评测吞吐量场景下扩展性更好，也更容易按节点做过滤和采样，控制 LLM Judge 成本。更重要的是，Langfuse 后续要支持的 multi-span evaluation 也会建立在 observation-first 模型上，所以官方选择把评测体系统一到底层 Observation，而不是同时长期维护 Trace 和 Observation 两套逻辑。换句话说，Trace 这个“调用链概念”仍然存在，只是它不再作为主要的数据和评测承载实体；如果你想做过去的“整条 Trace 评测”，现在通常就是评 Root Observation，因为它承载整个请求的总 input/output。([Langfuse](https://langfuse.com/faq/all/llm-as-a-judge-migration?utm_source=chatgpt.com "How do I upgrade trace-level evaluators to observation-level evaluators? - Langfuse"))
...
> **Annotation Queue（标注队列）** 是一个**“人工评审任务工作流”**管理工具。
>
> 简单来说，它就像是你为 **“人工评估 LLM 输出质量”** 专门建的一个 **“待办事项清单（Todo List）”**。它把散落在日志里的 Trace（追踪链路）集中起来，分配给标注员（可以是你的同事，也可以是你自己），进行人工打分和错误标记。
